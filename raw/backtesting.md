...
# Backtesting & Forward Testing — Research Notes
...
- **Rolling**: IS window slides forward at fixed length. Adapts to
  regime change but loses old data.
...
| Survivorship (using current S&P 500 to backtest 1990) | Use point-in-time constituent list |
...
- **Trade-order shuffle**: randomise trade sequence, re-compute
  equity curve.
...
- **White's Reality Check** — tests whether a strategy's OOS
  performance is better than chance, accounting for data-snooping.
- **Hansen's SPA Test** — Superior Predictive Ability; more powerful
  than White's RC for multiple strategies.
...
- Bailey, Borwein, López de Prado, Zhu, *Pseudo-mathematics and
  financial charlatanism*, 2014 (backtest overfitting critique).
